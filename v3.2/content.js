/**
 * LinkedIn Engagement Highlighter V3.2 - Content Script
 *
 * Major improvements over V3:
 * - Fixed feed detection: merges results from ALL selector strategies
 *   instead of bailing after the first match. Adds action-bar-anchor
 *   strategy that finds Like/Comment/Repost/Send buttons and walks up.
 * - Rich export: Author name, Date, Hook (first 3 lines), Media type
 *   (Image / Video / Document / Carousel / Text-only).
 * - Export header shows author name, not generic "LinkedIn Top Post".
 *
...
(function () {
  ...
  /**
   * Returns the main content column so we never match sidebar / nav.
   */
  ...
  /**
   * Unified post finder.  Unlike V3, this **merges** results from every
   * strategy instead of returning after the first hit — so we never miss
   * posts that one selector catches but another doesn't.
   */
  function findAllPosts() {
    ...
    // ── Strategy 6 (NEW — action-button anchor) ──────────────────────
    //  Find buttons whose visible text is exactly "Like" and walk up to
    //  the nearest post-sized container.  This is the most resilient
    //  strategy because LinkedIn always renders these four buttons.
    main.querySelectorAll("button").forEach((btn) => {
      const label = (btn.textContent || "").trim();
      // LinkedIn renders "Like" as the first action for every post
    ...
    //  Look for spans/buttons whose text matches "N comments" or
    //  "N reposts" — these exist in every post's social-counts row.
  ...
  /**
   * Walk up the DOM from `startEl` looking for the post container.
   * Stops at `boundary` (the main column) or after `maxSteps`.
   */
  function walkUpToPost(startEl, boundary, maxSteps) {
    ...
    for (let i = 0; i < maxSteps && el && el !== boundary && el !== document.body; i++) {
      el = el.parentElement;
      if (!el) break;
      // Strong signals: LinkedIn data attributes or known class
      if (
    ...
    // Fallback: walk up again looking for a "large enough" container
    // that sits inside the main column and has social action buttons
    el = startEl;
    for (let i = 0; i < maxSteps && el && el !== boundary && el !== document.body; i++) {
      el = el.parentElement;
      if (!el) break;
      if (el.offsetHeight >= 150 && el.offsetWidth >= 300) {
        // Confirm this container has at least one button
  ...
  /**
   * Remove nested containers and exact duplicates.
   */
  function deduplicatePosts(elements) {
    // Sort by DOM depth (shallowest first) so that when we filter nested
    // elements we keep the outermost container.
  ...
  function extractEngagement(postInfo) {
    ...
    if (countsEl) {
      ...
      // Reactions fallback: first numeric-only span that isn't comments/reposts
      if (reactions === 0) {
      ...
      // Comments & reposts: scan text in the counts container
  ...
  function extractPostDate(postEl) {
    ...
    // Actor sub-description (e.g. "2d · Edited")
    ...
    // Broad scan for short relative-time spans
  ...
  function extractPostMeta(postInfo) {
    ...
    if (actorEl) {
      ...
      // Remove "View ___'s profile" sr-only text that LinkedIn injects
      displayName = displayName.replace(/View .+'s profile/i, "").trim();
    }

    // Profile link for username slug
    ...
    // Activity page — profile owner is the author
    if (!username) {
    ...
    // ── Caption (full text, truncated for display) ──────────────────
  ...
  /**
   * Returns the first 3 non-empty lines of the post text (the "hook").
   */
  ...
  /**
   * Detects the type of media attached to a post.
   * Returns "image" | "video" | "document" | "carousel" | "article" | "text"
   */
  function detectMediaType(postEl) {
    // Video (check first — videos also contain poster images)
    if (
    ...
    // Document / PDF (carousel of slides)
    if (
      postEl.querySelector(
    ...
    if (
      ...
      // Only count as article if there's an article card, not just any link
    ...
    // Image — look for content images, skip profile pics and emoji icons
    ...
    // Fallback: look for large <img> elements that aren't profile pics
    for (const img of postEl.querySelectorAll("img")) {
      const src = (img.getAttribute("src") || "").toLowerCase();
      const cls = (img.getAttribute("class") || "").toLowerCase();
      // Skip profile pics, emoji icons, reaction icons
      if (
        ...
        continue;
      // If the image looks like content (reasonably large or has media class)
    ...
    panel.innerHTML = `
      <div class="leh-panel-header">
        <div class="leh-header-left">
          ...
          <span class="leh-version">v3.2</span>
        </div>
  ...
  // ════════════════════════════════════════════════════════════════════
  //  EXTRACT & EXPORT  (V3.2 — author name, date, hook, media type)
  // ════════════════════════════════════════════════════════════════════
  ...
  /**
   * V3.2 export modal with: Author, Hook, Date, Media Type columns.
   * Header shows author name context, not generic "LinkedIn Top Post".
   */
  function showExportModal(posts, totalScored) {
    ...
    posts.forEach((p, i) => {
      ...
      const hookHtml = p.hook
        ? '<span class="leh-modal-hook">' + esc(p.hook).replace(/\n/g, "<br>") + "</span>"
      ...
      rows +=
        ...
        "</tr>" +
        '<tr class="leh-hook-row"><td></td><td colspan="8">' + hookHtml + "</td></tr>";
    });

    // Build title from top author if available
    ...
    // ── Copy Text ── (V3.2: author name as header, date, hook, media)
    ...
    // ── Copy JSON ── (V3.2: includes hook, mediaType)
    ...
    // ── Download CSV ── (V3.2: includes hook, media type)
    document.getElementById("leh-download-csv").addEventListener("click", () => {
      const esc = (v) => { const s = String(v ?? ""); return s.includes(",") || s.includes('"') || s.includes("\n") ? '"' + s.replace(/"/g, '""') + '"' : s; };
      const hdr = "Rank,Author,Date,Reactions,Comments,Reposts,Score,Hook,Media Type,Post URL";
  ...
  function saveSettings() {
    try {
      chrome.storage.local.set({
        lehSettingsV32: { enabled, showScores, mode, absoluteThreshold, weights, autoScrollSpeed, dateFilter, customDateFrom, customDateTo },
      });
  ...
  function loadSettings() {
    try {
      chrome.storage.local.get("lehSettingsV32", (res) => {
        if (!res?.lehSettingsV32) return;
        const s = res.lehSettingsV32;
  ...
  function init() {
    ...
    // Third pass to catch late-loading posts
